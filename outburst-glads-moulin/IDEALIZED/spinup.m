...
if any(steps==1)
    ...
    md = triangle(model, 'ideal3_downsamp_3.exp', 20);

    %pos = find(md.mesh.y < 4.0233e6); % This modifies the terminus base
    %md.geometry.base(pos) = 2.3845e3;
...
if any(steps==2)
    ...
    md=parameterize(md,'idealized.par');
	...
	% Initial subglacial gapht of 0.01m everywhere
...
if any(steps==3);
	...
	md.timestepping.time_step=43200/md.constants.yts; % Time step (in years), 1800=1800s=30min
	md.timestepping.final_time=300/365; % Final time (in years), 30/365=30 days
