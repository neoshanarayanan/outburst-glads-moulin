% Coupled outburst flood using GlaDS
% Neosha Narayanan, February 2026

load('../Models/kyagar62_wsu_coupled.mat')
md = md;

%% Set initial conditions (from SHAKTI right now)
% Set new initial conditions (starting from end of previous run)
loading_time = length(md.results.TransientSolution);
% 
% md.hydrology.head=md.results.TransientSolution(loading_time-1).HydrologyHead;
% md.hydrology.gap_height=md.results.TransientSolution(loading_time-1).HydrologyGapHeight;
% md.hydrology.reynolds=md.results.TransientSolution(loading_time-1).HydrologyBasalFlux./1.787e-6;
% md.friction.effective_pressure = md.results.TransientSolution(loading_time).EffectivePressure;
md.geometry.surface = md.geometri.base + md.geometry.thickness;
...
md.initialization.pressure = md.results.TransientSolution(loading_time-1).Pressure;
md.initialization.channelarea = md.results.TransientSolution(loading_time-1).ChannelArea;
md.initialization.hydraulic_potential = md.results.TransientSolution(loading_time-1).HydraulicPotential;
md.initialization.watercolumn = md.results.TransientSolution(loading_time-1).HydrologySheetThickness;


md.initialization.vel = md.results.TransientSolution(loading_time-1).Vel;
md.initialization.vx = md.results.TransientSolution(loading_time-1).Vx;
md.initialization.vy = md.results.TransientSolution(loading_time-1).Vy;

%% Define model timing 

year_start_time = 0/365;
year_end_time = 90/365;
...
description = 'long leadup GLOF, starting from kyagar14_coupled_wsu_atmpos_C1.mat with , max flux=0.25,  max gap height = 100m';
save('kyagar14_coupled_outburst_newsb.mat', 'md', 'description','pos', '-v7.3')
